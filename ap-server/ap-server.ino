...
void setup() {
  ...
  SerialGPS.begin(38400,SERIAL_8E2);
...
void loop() {
  ...
  for (i = 0; i < MAX_SRV_CLIENTS; i++) {
    if (tcpServerClients[i] && tcpServerClients[i].connected()) {
      ...
      while ((bytesAvail = tcpServerClients[i].available()) > 0) {
        bytesIn = tcpServerClients[i].readBytes(writebuffer, min(sizeof(writebuffer), bytesAvail));
        if (bytesIn > 0) {
          SerialGPS.write(writebuffer, bytesIn);
          delay(0);
        }
      }
    }
  ...
  while ((bytesAvail = SerialGPS.available()) > 0) {
    bytesIn = SerialGPS.readBytes(readbuffer, min(sizeof(readbuffer), bytesAvail));
    if (bytesIn > 0) {
      //push UART data to all connected telnet clients
      for (i = 0; i < MAX_SRV_CLIENTS; i++) {
        if (tcpServerClients[i] && tcpServerClients[i].connected()) {
          tcpServerClients[i].write((uint8_t*)readbuffer, bytesIn);
          delay(0);
        }
      }
    }
